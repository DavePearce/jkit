...
//     gen.tokensStart();
    for (Enumeration _enum = prods.elements(); _enum.hasMoreElements();) {
...
//     gen.tokensEnd();
  ...
  private static void emitNormalProductions(Generator gen, Vector prods) {
    ...
    for (Enumeration _enum = prods.elements(); _enum.hasMoreElements();) {
      NormalProduction np = (NormalProduction)_enum.nextElement();
...
//     gen.text("[<-" + exp.getClass().getName() + "]");
  ...
  private static void emitExpansionChoice(Choice c, Generator gen) {
    for (java.util.Enumeration _enum = c.choices.elements();
  ...
  private static void emitExpansionSequence(Sequence s, Generator gen) {
    ...
    for (java.util.Enumeration _enum = s.units.elements();
	 _enum.hasMoreElements();) {
      Expansion e = (Expansion)_enum.nextElement();
  ...
  private static void emitRE(RegularExpression re, Generator gen) {
	...
	gen.text("~");
      ...
      gen.text("[");
      for (java.util.Enumeration _enum = cl.descriptors.elements();
	   _enum.hasMoreElements();) {
	...
	if (_enum.hasMoreElements()) {
      ...
      for (java.util.Enumeration _enum = c.choices.elements();
	   _enum.hasMoreElements();) {
	RegularExpression sub = (RegularExpression)(_enum.nextElement());
	emitRE(sub, gen);
	if (_enum.hasMoreElements()) {
	  gen.text(" | ");
	}
  ...
  private static String v2s(Vector v, boolean newLine) {
    ...
    for (Enumeration _enum = v.elements(); _enum.hasMoreElements();) {
      Token tok = (Token)_enum.nextElement();
